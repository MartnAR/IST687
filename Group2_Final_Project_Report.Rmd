...
```{r tidy = TRUE}
premTransformed$win <- as.factor(ifelse(premTransformed$GS > premTransformed$GA, 1, 0))
...
logModel <- glm(formula = win ~ HomeAway + FTR + HTdiff + HTR + Referee + ShotsTaken + ShotsAllowed + ShotsOnTarget + OppShotsOnTarget + FoulsCommitted + FoulsReceived + CornerKicks + OppCornerKicks
+                                   + YellowCards
+                                   + OppYellowCards
+                                   + RedCards
+                                   + OppRedCards
...
+                                   + red_yellow
+                                   + opp_red_yellow,
, data = premTransformed, family = binomial(logit))
summary(logModel)
